...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_name', type=str, default='resnet18_cifar10_128_0.5_200_64_800_1_1_[[70, 70, 80], [65, 65, 65], [65, 65, 65]]_[[1, 10], [1, 9], [0, 10]]_[0, 1, 2]_model_550.pth',
                        help='The pretrained model path')
    parser.add_argument('--dataset_name', type=str, default='cifar10', help='the downstream task')
    parser.add_argument('--sample', type=float, default=0.1, help='the downstream task')
    parser.add_argument('--poison_ratio', type=float, default=0, help='the downstream task')
    ...
    if dataset_name == "gtsrb":
        ...
        if dataset_name == "cifar10":
            ...
            if main_task == "cifar100":
                train_data = CIFAR_100(root='../SimCLR/data/', train=False, sample=sample, is_train=True, poison_ratio=poison_ratio, transform=cifar100_test_transform, download=True)
            else:
                train_data = CIFAR_100(root='../SimCLR/data/', train=False, sample=sample, is_train=True, poison_ratio=0, transform=cifar100_test_transform, download=True)
            test_data = CIFAR_100(root='../SimCLR/data/', train=False, transform=cifar100_test_transform, download=True)
            test_data_poison = CIFAR_100(root='../SimCLR/data/', train=False, poison_ratio=1,transform=cifar100_test_transform, download=True)
        elif dataset_name == "svhn":
            if main_task == "svhn":
                train_data = mySVHN(root='../SimCLR/data/', split="test", sample=sample,is_train=True,poison_ratio=poison_ratio, transform=svhn_test_transform, download=True)
            else:
                train_data = mySVHN(root='../SimCLR/data/', split="test", sample=sample,is_train=True,poison_ratio=0, transform=svhn_test_transform, download=True)
            test_data = mySVHN(root='../SimCLR/data/', split="test", transform=svhn_test_transform, download=True)
            test_data_poison = mySVHN(root='../SimCLR/data/', split="test", poison_ratio=1,transform=svhn_test_transform, download=True)
        elif dataset_name == "mnist":
            train_data = myMNIST(root='../SimCLR/data/', train=False, sample=sample, transform=mnist_test_transform, download=True)
            test_data = myMNIST(root='../SimCLR/data/', train=False, transform=mnist_test_transform, download=True)
            test_data_poison = myMNIST(root='../SimCLR/data/', train=False, poisoned=True,transform=mnist_test_transform, download=True)
