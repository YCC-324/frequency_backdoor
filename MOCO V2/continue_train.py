...
import sys
from utils import *
from load_cifar10_pair2 import *
...
if __name__ == '__main__':
    ...
    parser.add_argument('--start_epoch', default=0, type=int, help='Momentum used for the update of memory bank')
    # args parse
    ...
    model_q_path = save_dir+'/{}_model.pth'.format(save_name_pre)

    sd = torch.load(model_q_path)
    new_sd = model_q.state_dict()
    new_names = [v for v in new_sd]
    # old_names = [v for v in sd]
    for i, j in enumerate(new_names):
        new_sd[j] = sd[j]
    model_q.load_state_dict(new_sd)

    model_k_path = save_dir+'/{}_model_k.pth'.format(save_name_pre)
    new_sd = model_k.state_dict()
    new_names = [v for v in new_sd]
    # old_names = [v for v in sd]
    for i, j in enumerate(new_names):
        new_sd[j] = sd[j]
    ...
    for param_k in model_k.parameters():
