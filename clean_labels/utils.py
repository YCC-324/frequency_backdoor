...
def read_config_verify():
    f = open('./config_verify.txt', encoding="utf-8")
    content = f.read()
    #print(content)
    params = json.loads(content)
    return params
...
def read_config_svd():
    f = open('./config_svd.txt', encoding="utf-8")
    content = f.read()
    #print(content)
    params = json.loads(content)
    return params
...
def validate(model, epoch, valloader, criterion, checkpoint=None):
    ...
    for i, (input, target) in enumerate(valloader):
        ...
        print()
        print(target_np)
        # print(np.argmax(target_np, axis = -1))
        #print()
        '''
    ...
    torch.save(model.state_dict(), 'checkpoints/%s.t7' % checkpoint)

def validate_target(model, valloader, target_label):
    # switch to evaluate mode
    model.eval()
    ...
    for i, (input, target) in enumerate(valloader):
        input = torch.squeeze(input)
        target = torch.squeeze(target)
        indexs = [index for (index,target_i) in enumerate(target) if target_i==target_label]
        if len(indexs) == 0:
            continue
        input = input[indexs]
