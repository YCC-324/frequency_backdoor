...
def get_cifar_loaders(data_loc='./disk/scratch/datasets/MNIST/', batch_size=128, cutout=True, n_holes=1, length=16):
    num_classes = 10
    ...
    trainset = torchvision.datasets.MNIST('./disk/scratch/datasets/MNIST/', train=True, download=True, transform=transform_train)
    trainloader = torch.utils.data.DataLoader(trainset, batch_size=batch_size, shuffle=True, num_workers=2)

    testset = torchvision.datasets.MNIST('./disk/scratch/datasets/MNIST/', train=False, download=True, transform=transform_test)
...
def validate(model, epoch, valloader, criterion, checkpoint=None):
    ...
    if checkpoint:
        ...
        torch.save(state, 'checkpoints/done%s.t7' % checkpoint)
