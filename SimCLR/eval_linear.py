...
if __name__ == '__main__':
    ...
    parser.add_argument('--add_transform', type=bool, default=False, help='Number of sweeps over the dataset to train')
    parser.add_argument('--target_label', type=int, default=1, help='Number of sweeps over the dataset to train')
    ...
    if dataset_name == "gtsrb":
        ...
        if dataset_name == "cifar10":
            ...
            if add_transform:
                test_data_transform = CIFAR_10(root='../SimCLR/data', train=False, transform=cifar10_test_transform, add_transform=True, download=True)
                test_data_poison_transform = CIFAR_10(root='../SimCLR/data', train=False, poison_ratio=1,transform=cifar10_test_transform, add_transform=True, download=True)
        ...
        if add_transform:
            test_loader_transform = DataLoader(test_data_transform, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True)
            test_loader_poison_transform = DataLoader(test_data_poison_transform, batch_size=batch_size, shuffle=False, num_workers=16, pin_memory=True)
    ...
    target_acc_1, target_acc_5 = val_target(model, test_loader, target_label)
    print("target ACC: ",target_acc_1)
    ...
    if filter:
        ...
        target_acc_1, target_acc_5 = val_target(model, test_loader_blur, target_label)
        print("target ACC blur: ",target_acc_1)
        ...
        target_acc_1, target_acc_5 = val_target(model, test_loader_gaussian, target_label)
        print("target ACC gaussian: ",target_acc_1)
        ...
        target_acc_1, target_acc_5 = val_target(model, test_loader_median, target_label)
        print("target ACC median: ",target_acc_1)        
        ...
        target_acc_1, target_acc_5 = val_target(model, test_loader_svd, target_label)
        print("target ACC svd: ",target_acc_1)   
    ...
    # if add_transform:
    #     test_loss, test_acc_1, test_acc_5 = train_val(model, test_loader_transform, num_class, None, loss_criterion, 0, 0)
    #     print("-------------------------------------")
    #     print("ACC transform: ", test_acc_1)
    #     poison_loss, test_asr_1, test_asr_5, target_info = train_val(model, test_loader_poison_transform, num_class, None, loss_criterion, 0, 0, poisoned=True)
    #     print("ASR transform: ", test_asr_1)             